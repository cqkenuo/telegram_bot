...
def search(search_key):
    ...
    try:
        parsed = BeautifulSoup(str(driver.find_element_by_class_name('site-wrapper')
                                   .get_attribute('innerHTML')), 'lxml')
        ...
        from log import log
        log('web_scraping.py.search()', 'Unspecified exception occurred', True)
        pass
...
def r34(search_key, limit=20):
    url = 'https://rule34.xxx/index.php?page=dapi&s=post&q=index&limit={}&{}'.format(limit,
                                                                                     urlencode({'tags': search_key}))
    ...
    try:
        ...
        from log import log
        log('web_scraping.py.r34()', 'Unspecified exception occurred', True)
        pass
...
def live_leak(search_term):
    ...
    try:
        ...
        from log import log
        log('web_scraping.py.live_leak()', 'Unspecified exception occurred', True)
        pass
...
def my_bb():
    ...
    try:
        parsed = BeautifulSoup(str(driver.find_element_by_class_name('feed_article_container')
                                   .get_attribute('innerHTML')), 'lxml')
        ...
        for a in results:
            try:
                ...
                # Filter the URL's for valid formats only
        ...
        from log import log
        log('web_scraping.py.my_bb()', 'Unspecified exception occurred', True)
        pass
...
def site_data(url):
    try:
        ...
        from log import log
        log('web_scraping.py.site_data()', 'Unspecified exception occurred', True)
        return None
...
def get_chrome_driver(url):
    ...
    try:
        ...
        from log import log
        log('web_scraping.py.get_chrome_driver()', 'Unspecified error', True)
        return None
